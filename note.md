...
leetcode561:
这道题我们要将数组里的数字两两分组，并且保证每组数里面的最小的数是当前最大的。
我们只能尽可能的先将小的和很小的数安排在一个组里，这样我们首先对数组进行排序，排序后按照顺序两两组合就可以了。

leetcode575:
这道题给的数组已知有多少个糖果，其中这里每一个元素代表这个糖果属于第几个类别的，
也就是说我们不知道一共有多少种，我们需要记录出现的不同种类，这里时可以使用hash表的。

leetcode409:
这道题让我们使用给出的字母来看最多能够构成多长的回文串，对于偶数个出现的无需多考虑，对于奇数个出现的，
我们我们可以采纳这里面的n-1偶数个，然后把多余的取出来作为中间就可以了。

leetcode109:
这道题我们需要比较数之间的关系，对于两个字符串来说，我们需要按位来比较，比如987和923，由于第二位8更大，所以我们直接取983为大就可以。例如913和98，由于第二位的8更大，所以我们取98即可。

leetcode456:
这道题的意思是，我们需要找到多个区间的最小的共同区间，并且射一箭。
我们可以发现，射箭的位置一定是某个区间的右端点，所以我们可以通过对右端点进行升序排序，从最小的右端点开始判断
如果下一个区间左端点在射箭的左边，则看下一个，直到左端点不在射箭位置，++。然后找当前的右端点位置，
